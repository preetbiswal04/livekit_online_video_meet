...
# --- Start Local MongoDB ---
Write-Host "Starting Local MongoDB on port 27018..." -ForegroundColor Green
$mongoPath = "C:\Program Files\MongoDB\Server\8.2\bin\mongod.exe"
$dbPath = Join-Path $PSScriptRoot "mongo_data"

if (Test-Path $mongoPath) {
    if (-not (Test-Path $dbPath)) {
        New-Item -ItemType Directory -Path $dbPath | Out-Null
    }
    # Start mongod in a new process, listening on 27018
    Start-Process -FilePath $mongoPath -ArgumentList "--port 27018", "--dbpath `"$dbPath`"" -NoNewWindow
} else {
    Write-Host "Error: mongod.exe not found at $mongoPath" -ForegroundColor Red
}
